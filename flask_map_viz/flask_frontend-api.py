...
import textblob_analysis as txtAnalyse
...
def publish_tweets(topicname):
    ...
    def event():
        for tweet in consumer_2:
            ...
            # Get message of tweets properly cleaned.
            # print("message tweet (without cleaning):\n", tweet_dict['text'])
            message_tweet = nlp_cleaning_pipeline(tweet_dict['text'].strip())
            # print("message tweet cleaned:\n", message_tweet)

            # English tweets processing
            if tweet_dict['lang'] == 'en':
                ...
                tweet_filtered_en['sentiment'] = sentiment_status
                print("tweet_filtered_en:\n", tweet_filtered_en)  # type Dict.

                # https://stackoverflow.com/questions/11875770/how-to-overcome-datetime-datetime-not-json-serializable
                tweet_filtered_str = json.dumps(tweet_filtered_en, default=str)
                print("tweet_filtered_str:\n", tweet_filtered_str)

                # Post tweets to PowerBI in 'Str' format.
                post_to_powerbi(tweet_filtered_str)

                print("tweet_filtered_en:\n", tweet_filtered_en)
                continue

            # French tweets processing
            else:
                ...
                print("tweet_filtered_fr:\n", tweet_filtered_fr)

                # Transform Dict in Str before to publish.
                tweet_filtered_str = json.dumps(tweet_filtered_fr, default=str)

                # Publish my tweets via an API.
...
# Sentiment analysis of twets (message).
def sentiment_analysis(message: str):
    return txtAnalyse.sentiment_analysis(message)
